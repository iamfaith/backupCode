...
*/
    ...
    private Response unzip(Response response) throws IOException {
        ...
        Headers strippedHeaders = response.headers().newBuilder().removeAll("Content-Encoding").removeAll(HTTP.CONTENT_LEN).build();
    ...
    public static boolean hasBody(Response response) {
        ...
        if (OkHeaders.contentLength(response) != -1 || HTTP.CHUNK_CODING.equalsIgnoreCase(response.header(HTTP.TRANSFER_ENCODING))) {
            return true;
        }
        return false;
    }
    ...
    private Request networkRequest(Request request) throws IOException {
        ...
        if (request.header(HTTP.TARGET_HOST) == null) {
            result.header(HTTP.TARGET_HOST, Util.hostHeader(request.url(), false));
        }
        if (request.header(HTTP.CONN_DIRECTIVE) == null) {
            result.header(HTTP.CONN_DIRECTIVE, HTTP.CONN_KEEP_ALIVE);
        }
        ...
        if (!cookies.isEmpty()) {
            result.header(SM.COOKIE, cookieHeader(cookies));
        }
        if (request.header(HTTP.USER_AGENT) == null) {
            result.header(HTTP.USER_AGENT, Version.userAgent());
        }
        return result.build();
    }
                ...
                if (this.forWebSocket) {
                    ...
                    if (this.sentRequestMillis == -1) {
                        if (OkHeaders.contentLength(this.networkRequest) == -1 && (this.requestBodyOut instanceof RetryableSink)) {
                            this.networkRequest = this.networkRequest.newBuilder().header(HTTP.CONTENT_LEN, Long.toString(((RetryableSink) this.requestBodyOut).contentLength())).build();
                        }
                        this.httpStream.writeRequestHeaders(this.networkRequest);
                    }
    ...
    private Response readNetworkResponse() throws IOException {
        ...
        if (!(this.forWebSocket && networkResponse.code() == 101)) {
            networkResponse = networkResponse.newBuilder().body(this.httpStream.openResponseBody(networkResponse)).build();
        }
        if ("close".equalsIgnoreCase(networkResponse.request().header(HTTP.CONN_DIRECTIVE)) || "close".equalsIgnoreCase(networkResponse.header(HTTP.CONN_DIRECTIVE))) {
    ...
    private static Headers combine(Headers cachedHeaders, Headers networkHeaders) throws IOException {
        ...
        for (i = 0; i < size; i++) {
            ...
            if (!HTTP.CONTENT_LEN.equalsIgnoreCase(fieldName) && OkHeaders.isEndToEnd(fieldName)) {
